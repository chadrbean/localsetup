...
| PR has no `jenkins/<pipeline>` status | Read the job's `indexing/events.log` (under `JENKINS_HOME/jobs/<repo>/jobs/<job>/`). `No automatic build triggered for PR-N` means a build strategy rejected it: check `seed.groovy` `buildStrategies`. Before 2026-09-26, `skipInitialBuildOnFirstBranchIndexing` alone rejected every PR's first commit. No `Received Pull request` line means a webhook problem (next row). |
